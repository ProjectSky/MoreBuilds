    ...
    for i = 0, gs:getSpecialObjects():size() - 1 do
        ...
        if o and instanceof(o, 'IsoThumpable') then
            if not o:haveFuel() then
                if o:getName() == getText 'ContextMenu_Light_Pole' or o:getName() == 'Light Pole' then
                    return o
                end
            end
        end
...
BuildCommands.object.toggleElectricLight = function(player, args)
    ...
    if o then
        if
            o:getSquare():haveElectricity() or
...
function MoreBuildClientCommands.OnDestroyIsoThumpable(o, player)
    ...
    if not o:getSquare() or not (o:getName() == getText 'ContextMenu_Light_Pole' or o:getName() == 'Light Pole') then
        return
    end
