...
MoreBuild.tooltipCheckForMaterial = function(player, material, amount, tooltip)
  ...
  local groundItem = buildUtil.checkMaterialOnGround(playerObj:getCurrentSquare())
  if amount > 0 then
    invItemCount = playerInv:getItemCountFromTypeRecurse(material)

    if material == "Base.Nails" then
      invItemCount = invItemCount + playerInv:getItemCountFromTypeRecurse("Base.NailsBox") * 100
    ...
    if material == "Base.Screws" then
      invItemCount = invItemCount + playerInv:getItemCountFromTypeRecurse("Base.ScrewsBox") * 100
